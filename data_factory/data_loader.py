...
class AIOPSSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        self.mode = mode
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load(data_path + "/AIOPS_train.npy")
        data = data.reshape(-1,1)
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load(data_path + "/AIOPS_test.npy")
        test_data = test_data.reshape(-1,1)
        self.test = self.scaler.transform(test_data)
        self.train = data
        data_len = len(self.train)
        self.val = self.train[(int)(data_len * 0.7):]
        self.test_labels =( np.load(data_path + "/AIOPS_test_label.npy")).reshape(-1,1)
...
class WSDSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        self.mode = mode
        self.step = step
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load(data_path + "/WSD_train.npy")
        data = data.reshape(-1,1)
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load(data_path + "/WSD_test.npy")
        test_data = test_data.reshape(-1,1)
        self.test = self.scaler.transform(test_data)
        self.train = data
        data_len = len(self.train)
        self.val = self.train[(int)(data_len * 0.7):]
        self.test_labels = (np.load(data_path + "/WSD_test_label.npy")).reshape(-1,1)
...
class YahooSegLoader(object):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        data_len = len(self.train)
        self.val = self.train[(int)(data_len * 0.7):]
        self.test_labels =( np.load(data_path + "/Yahoo_test_label.npy")).reshape(-1,1)
...
class UniDataset(torch.utils.data.Dataset):
    ...
    def __getsamples(self, values, labels, missing):
        ...
        Y = torch.zeros((self.sample_num, self.window,1))
        ...
        for cnt in range(len(values)):
            ...
            for j in range(len(v) - self.window + 1):
                ...
                Y[i, :, 0] = torch.from_numpy(np.asarray(l[j : j + self.window]))
    ...
    def __len__(self):
        if(self.mode=='test' or self.mode=='thre'):
            return self.sample_num//self.window
...
def get_loader_segment(data_path, batch_size, win_size=100, step=100, mode='train', dataset='KDD'):
    if (dataset == 'SMD'):
        ...
        dataset = UniDataset(win_size,data_path,'w',mode,1,0)
    elif (dataset == 'WSD'):
        dataset = UniDataset(win_size,data_path,'w',mode,1,0)
    elif (dataset == 'Yahoo'):
        dataset = UniDataset(win_size,data_path,'w',mode,1,0)
